...
- **流量**：網路無法存取；5月1日報稅開放前 ≈ 14 天（黃金窗口最後衝刺）
- **前次備注確認**：session 64 完成 /medical-deduction-2026 + 4篇 Threads。本次執行 session 64 列出的下一步：`/disaster-deduction-2026`（完成列舉扣除額6項全覆蓋）。

### 今天做了什麼

#### 1. 新頁面：`/disaster-deduction-2026`（災害損失列舉扣除額完整攻略）

**為什麼做這個**：
列舉扣除額系列最後一個缺口。台灣颱風/地震頻繁，每年颱風季過後有大量「颱風損失可以報稅嗎」的高意圖搜尋。
最關鍵且反直覺的資訊：必須在**災後30天內**向國稅局申請損失勘查，不是5月報稅時才填。
這個「30天死線」是大多數人不知道、也因此白白損失節稅機會的核心資訊，分享動機強。
完成後，列舉扣除額6項全部覆蓋：捐款、保險費、醫藥費、災害損失、房貸利息、房租。

**頁面內容**（5個章節）：
| 章節 | 主題 | 核心內容 |
|---|---|---|
| 章節1 | 哪些損失可以申報 | 颱風/地震/火災/洪水；不包含竊盜/個人疏失 |
| 章節2 | 30天期限（重點） | 災後30天內向國稅局申請勘查，錯過喪失資格 |
| 章節3 | 損失金額計算+省稅試算 | 三個情境（車輛泡水、裝潢全毀、火災家電）|
| 章節4 | eTax申報步驟 | 前置作業 → 報稅填報 → 文件保存5年 |
| 章節5 | 注意事項與節稅策略 | 只有列舉才有效、扶養親屬損失可合併、超過所得怎麼辦 |
- FAQ JSON-LD（5題）+ Article schema
- AdUnit × 2 + TaxAffiliateCTA × 1
- 9個相關工具內連（完整列舉扣除額系列互相串聯）

**SEO 關鍵字覆蓋（12個）**：
災害損失扣除額、颱風損失報稅、地震損失申報、火災損失抵稅、天災損失扣除、114年災害損失申報、水災損失報稅、災害損失列舉扣除、颱風損失節稅、不可抗力損失扣除、災害損失所得稅、房屋損失報稅

#### 2. Sitemap 更新
- 新增 `/disaster-deduction-2026`（priority: 1.0，changeFrequency: yearly，lastModified: 2026-04-17T14:00:00Z）

#### 3. Threads 草稿 × 4（災害損失系列）
**`public/threads-drafts/2026-04-17-disaster-deduction.md`**

| 草稿 | 角度 | 核心鉤子 | 建議發布日 |
|---|---|---|---|
| A（30天死線震撼版） | 「期限」的衝擊性 | 「不是等到5月才填，是災後30天內要去國稅局」 | 4/17 晚上8點 |
| B（車輛泡水省稅試算版） | 具體金額試算 | 「車子市價50萬、理賠40萬，可省2萬稅」 | 4/18 中午 |
| C（列舉扣除額完整系列收尾版） | 6項全覆蓋盤點 | 「台灣列舉扣除額6項，你用到幾項？」 | 4/20 早上9點 |
| D（地震震壞房子版） | 情感共鳴 | 「以為只能自認倒楣？其實可以申報」 | 4/21 晚上7點 |

#### 4. PTT/Dcard 論壇文
**`public/forum-posts/2026-04-17-disaster-loss-tax-ptt.md`**
- 完整攻略文，含省稅試算表、NG案例、操作步驟
- 建議發布至 PTT tax 板或 home-improvement 板

**Build 狀態**：node_modules 不存在（本地無法 build）→ Server Component、無 hooks，結構與現有頁面完全一致，push 至 Vercel 遠端 build 驗證

### 預期營收影響
- **新頁面 /disaster-deduction-2026**：
  - 「30天死線」是強分享動機，知道的人自然轉發給受災親友
  - 颱風過後的高意圖搜尋（每年7-9月颱風季後明顯放量），現在建頁面讓 Google 有時間收錄
  - 完成列舉扣除額6項全系列，內連網絡完整，提升整體 SEO 權重
  - 估計颱風季後帶入額外 300~600 訪客/月
- **Threads 4篇 + PTT 1篇**：
  - 草稿A的30天死線 = 衝擊性資訊，觸及率高
  - 草稿C的6項盤點 = 可收藏轉發的整理文
  - PTT 長文包含省稅試算表，適合在受災討論串回應「到底能省多少」

### 里程碑
✅ **列舉扣除額 6 項全覆蓋完成**（捐款、保險費、醫藥費、災害損失、房貸利息、房租）
✅ **特別扣除額 6 項全覆蓋完成**（薪資、身心障礙、幼兒學前、教育學費、儲蓄投資、長期照顧）
→ 台灣個人所得稅所有主要扣除項目已全部覆蓋，SEO 內容護城河建立完成。

### 下次要做的事（優先順序）
1. **[阻斷] Ian 設定 Vercel env `NEXT_PUBLIC_ADSENSE_SLOT`** — 廣告收入從 0 到有的關鍵
2. **[阻斷] Ian 申請 Money101 聯盟帳號** — CTA 等真實連結
3. **[立即] Ian 發 Threads 文**（按排程）：
   - 4/17 晚上8點：颱風損失30天死線（本次草稿 A）
   - 4/18 中午：車輛泡水省稅試算（本次草稿 B）
   - 4/20 早上：列舉扣除額6項盤點（本次草稿 C）
   - 4/21 晚上：地震震壞房子版（本次草稿 D）
   - 4/21：廟宇捐款迷思版（session 59 草稿）
4. **轉換率優化**（所有阻斷任務解除後的最高優先）：
   - 在 /tax-calculator 和 /deduction-compare 加「5月1日報稅開放倒數」urgency banner
   - 在高流量頁面（tax-calculator、income-tax-guide-2026）強化 affiliate CTA 點擊誘因
5. **新內容方向**（列舉/特別扣除額已全覆蓋，轉向其他高搜尋量缺口）：
   - `/114-tax-bracket-calculator`：互動式稅率計算器（目前 /income-tax-brackets 是靜態頁）
   - `/housing-subsidy-tax-2026`：青年購屋優惠相關稅務
   - 或集中在已有頁面的 SEO 優化（更新標題/描述、加強內連）
